...
void Renderer::drawFrame(const Game& G, float now) const {
    // draw stars
    ...
    if (G.state() == GState::MENU) {
        ...
        if (G.highScore() > 0) {
            char hs[64];
            snprintf(hs, sizeof(hs), "High Score:  %d", G.highScore());
            int hw2 = MeasureText(hs, 22);
            DrawText(hs, SW/2 - hw2/2, SH/2 + 80, 22, COL_GOLD);
        }
    }

    if (G.state() == GState::PLAYING || G.state() == GState::GAMEOVER) {
        // danger zone
        ...
        snprintf(hsBuf,    sizeof(hsBuf),    "BEST  %d",  G.highScore());

        int mins = (int)G.timeElapsed() / 60, secs = (int)G.timeElapsed() % 60;
